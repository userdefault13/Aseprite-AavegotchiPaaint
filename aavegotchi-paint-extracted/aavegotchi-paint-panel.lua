...
-- Export showPanel function so other commands can access it
_G.showAavegotchiPaintPanel = nil

-- Helper function to resolve script paths
local function resolveScriptPath(scriptName)
    if pluginPath then
        return app.fs.joinPath(pluginPath, scriptName)
    end
...
local function showPanel()
    ...
    -- Generate Sprite Sheet button (single collateral)
    panelDlg:button{
        id = "generate_sprite_sheet",
        text = "Generate Sprite Sheet",
        onclick = function()
            panelDlg:close()
            -- Load and run the sprite sheet generator
            _G._IS_LOADED_BY_MENU = true
            local scriptPath = resolveScriptPath("generate-aavegotchi-sprite-sheet.lua")
            dofile(scriptPath)
            if _G.showAavegotchiSpriteSheetDialog then
                _G.showAavegotchiSpriteSheetDialog()
            end
            _G._IS_LOADED_BY_MENU = nil
        end
    }
    
    -- Generate All Sprite Sheets button
    panelDlg:button{
        id = "generate_all_sprite_sheets",
        text = "Generate All Sprite Sheets",
        onclick = function()
            local assetsPath = panelDlg.data.assets_path or hardcodedAssetsPath
            local outputDir = assetsPath .. "/Output"
            
            -- Show confirmation dialog
            local confirmDlg = Dialog("Generate All Sprite Sheets")
            confirmDlg:label{
...
-- Export showPanel globally so other commands can access it
_G.showAavegotchiPaintPanel = showPanel

-- Function to show the main menu dialog with two options
local function showMainMenu()
    local menuDlg = Dialog("Aavegotchi Paint")
    menuDlg:label{
...
function init(plugin)
    -- Store plugin path for resolving script paths
