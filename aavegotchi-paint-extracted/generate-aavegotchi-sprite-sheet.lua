...
-- Function to show the sprite sheet generation dialog
local function showSpriteSheetDialog()
...
if app.fs.isDirectory(collateralsDir) then
    local files = app.fs.listFiles(collateralsDir)
    if files then
...
-- Register as a command (optional - for direct menu access)
function init(plugin)
    plugin:newCommand{
        id = "generate_aavegotchi_sprite_sheet",
        title = "Generate Aavegotchi Sprite Sheet",
        group = "tools",
        onenabled = function() return true end,
        onclick = showSpriteSheetDialog
    }
end

-- Export function for use by other scripts
_G.showAavegotchiSpriteSheetDialog = showSpriteSheetDialog

-- If this file is run directly (not via dofile from another script), show the dialog
-- This allows it to work both as a standalone command and when called from the menu
if not _G._IS_LOADED_BY_MENU then
